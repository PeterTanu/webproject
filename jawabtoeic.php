...
<?php         		
            ...
            for ($i=0;$i<$jumlah;$i++){
                //id nomor soal
          $nomor=$id_soal[$i];
			$query1 = mysqli_query($koneksi,"select * from soaltoeic where id_soal='$nomor'");
						...
						$urut = $urut + 1;
						echo '<tr><td valign="bottom">'.$urut.'. ';
				
                //jika user tidak memilih jawaban
                if (empty($pilihan[$nomor])){
                    $kosong++;
					$query1=mysqli_query($koneksi,"select * from soaltoeic where id_soal='$nomer'");
					$cek1=mysqli_num_rows($query1);
					if ($cek1){
						// output pertanyaan+jawaban(benar)
							if ($jwb=="a") {
							echo '<font color="red">'.$pertanyaan.'</font>';
							if (!empty($audio)){
								echo '<audio controls preload="none"><source src="',$audio.'"></audio>';
							}else{
							echo "";
							}
							echo '</td>
							<td rowspan=5 width="20%"><img src="'.$image.'" alt=" " width="200" height="200"></td></tr>
                    ...
                    $query=mysqli_query($koneksi,"select * from soaltoeic where id_soal='$nomer' and knc_jawaban='$jawaban'");
                    ...
                    if($cek){
						...
						$benar++;
							// output pertanyaan+jawaban(benar)
							if ($jawaban=="A") {
							echo '<font color="blue">'.$pertanyaan.'</font>';
							if (!empty($audio)){
								echo '<audio controls preload="none"><source src="',$audio.'"></audio>';
							}else{
							echo "";
...
header('location:ambilsoalt.php?info=Masukkan nama anda');
}
